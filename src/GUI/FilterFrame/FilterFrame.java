		...
		MainPanel.setLayout(new BorderLayout(0, 0));
			...
			addWindowListener(new WindowAdapter() {
				...
				public void windowClosing(WindowEvent e) {
					...
					if (DateFilterPanel.getDataPrecisaCmbBx().getGGMMAA().isEmpty()) {
						
						controllerLink.backFromFiltri();
						
					} else { 
						
						controllerLink.addQueryData("data_inizio = '" + DateFilterPanel.getDataPrecisaCmbBx().getGGMMAA() + "' OR "); 
						controllerLink.backFromFiltri();
					}
					
					if (DateFilterPanel.getInizioPeriodioCmbBx().getGGMMAA().isEmpty()) {
						
						controllerLink.backFromFiltri();
						
					} else {
						
						controllerLink.addQueryData("data_inizio BETWEEN '" + DateFilterPanel.getInizioPeriodioCmbBx().getGGMMAA() + "' AND '"+ DateFilterPanel.getFinePeriodoCmbBx().getGGMMAA() +"' OR ");
					}
