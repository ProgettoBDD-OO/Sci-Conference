...
public class InterventiDialog extends JDialog {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (PartecipantiLbl.getText().equals("Partecipanti (seleziona il numero):")) {
						...
						controller.addQueryModInterventi("UPDATE interventi SET ora_inizio = '"+ getOraInizio().gethhmm() +"',"
														  + " ora_fine = '"+ getOraInizio().gethhmm() + "',"
														  + " descrizione = '" + getDescrizione()  + "',"
														  + "WHERE id_intervento = '" + idSessione + "0" + n + "';");
					}
	...
	public boolean fieldsAreEmpty() {
		...
		if (getOraInizio().gethhmm().isEmpty() || getOraFine().gethhmm().isEmpty() || getDescrizione().isEmpty()) {
