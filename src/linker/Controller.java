...
import GUI.AdminFrames.Aggiunta2Frame;
import GUI.AdminFrames.GestioneFrame;
import GUI.AdminFrames.InfoDialog;
import GUI.AdminFrames.InfoModLbl;
import GUI.AdminFrames.InterventiDialog;
import GUI.AdminFrames.KeynoteDialog;
import GUI.AdminFrames.ModInterventoPanel;
import GUI.AdminFrames.ModSessionePanel;
import GUI.AdminFrames.Modifica2Frame;
import GUI.AdminFrames.OrganizzatoreDialog;
import GUI.AdminFrames.PartecipanteDialog;
import GUI.AdminFrames.SessioneDialog;
import GUI.CalendarFrame.CalendarCell;
...
import GUI.ConferenzaFrame.InsertPswrdDialog;
...
import myException.UsernameAlreadyExistsException;
...
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
	...
	public void goToLogIn() {
		...
		ViewLogIn = new LogInFrame(this);
		ViewLogIn.getPasswordFld().setText("%Ib8F27VNm^34_S5");
		ViewLogIn.setVisible(true);
	}
...
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
	...
	private void addInfoConferenza(String idConferenza) {
        ...
        ViewConferenza.getNorthPanel().setDataInizio(t.toDateFormat(conferenza.getDataInizio().getTime()));
        ViewConferenza.getNorthPanel().setDataFine(t.toDateFormat(conferenza.getDataFine().getTime()));

        ViewConferenza.getNorthPanel().setDescrizione(conferenza.getDescrizione());
	...
	public void addSessioneInfo(String idSessione, SessionePanel sessionePanel) {
		...
		sessionePanel.setDataInizio(t.toDateFormat(sessione.getDataInizio().getTime()));
		sessionePanel.setDataFine(t.toDateFormat(sessione.getDataFine().getTime()));
