...
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
	...
	String queryModificaInterventi = ""; //TODO:
	...
	public void goToModIntervento(int currentSession) {
		
		String IdSessione = sessioneDAO.getSessioni("1000").get(currentSession).getId();
		
		ViewInterventi = new InterventiDialog(this, IdSessione);
		setInfoIntervento(IdSessione);
		ViewInterventi.setLocationRelativeTo(null);
		ViewInterventi.setVisible(true);
	}
	//TODO capire come prendere l'indice della sessione tramite currentSession
	public void setInfoIntervento(String idSessione) {
		
		intervento = interventoDAO.getInterventi(idSessione).get(ViewModifica.getSessioniPanel().get(0).getSelezioneInterventoComboBox().getSelectedIndex());
    ...
    public int addModSessioneInfo(int currentSession, ModSessioniPanel sessionePanel) {
		...
		return addModInterventi(sessioni.get(currentSession).getId(), sessionePanel);
	}
	...
	public int addModInterventi(String idsessione, ModSessioniPanel sessionePanel) {
	...
	public void addQueryModInterventi(String q) throws EmptyFieldException, DateTimeException {
		...
		if (ViewInterventi.fieldsAreEmpty()) {
			...
			for (String p : ViewInterventi.getPartecipanti()) {
				...
				ViewModifica.getSessioniPanel().get(0).getInterventi().get(ViewModifica.getSessioniPanel().get(0).getSelezioneInterventoComboBox().getSelectedIndex()).getCentrPanel().add(new InfoModLbl(p));
			}
			ViewModifica.getSessioniPanel().get(0).getInterventi().get(ViewModifica.getSessioniPanel().get(0).getSelezioneInterventoComboBox().getSelectedIndex()).revalidate();
		}
	}
	
	
	public void addQueryModPartecipante(String q, int n) {
