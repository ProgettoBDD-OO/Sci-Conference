...
CREATE FUNCTION dopo_sesto_spazio(
    descrizione VARCHAR)
    RETURNS VARCHAR
    LANGUAGE 'plpgsql'
AS $BODY$
...
prima_parte INTEGER;
BEGIN
    prima_parte := LENGTH(prima_sesto_spazio(descrizione));
...
CREATE OR REPLACE FUNCTION filtered_research(
	nome_ricerca VARCHAR DEFAULT NULL,
	temi VARCHAR[] DEFAULT NULL,
	regioni VARCHAR[] DEFAULT NULL,
	data_precisa DATE DEFAULT NULL,
	inizio_periodo DATE DEFAULT NULL,
	fine_periodo DATE DEFAULT NULL)
...
BEGIN
    RETURN QUERY
		...
		WHERE nome ILIKE '%' || nome_ricerca || '%'
		  ...
		  AND (
                ($4 IS NOT NULL AND data_inizio = data_precisa)
