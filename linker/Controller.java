...
import GUI.UserFrames.LogInFrame;
import GUI.UserFrames.SignUpFrame;
import myException.*;
import myTools.*;
...
public class Controller {
	...
	int Mese1 = DataAttuale.get(Calendar.MONTH);
	int Mese2 = DataAttuale.get(Calendar.MONTH) + 1;
...
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
	...
	public void goToConferenza(String idConferenza) {
	    ...
	    addInfoConferenze(idConferenza);
	...
	private void addInfoConferenze(String idConferenza) {
        ...
        ViewConferenza.setDataInizio(t.toDateFormat(conferenza.getDataInizio().getTime()));
        ViewConferenza.setDataFine(t.toDateFormat(conferenza.getDataFine().getTime()));
                
        ViewConferenza.setDescrizione(conferenza.getDescrizione());
        ViewConferenza.setTema(conferenza.getTema());
        ViewConferenza.setCollocazione(collocazione.getSede() + ", " + collocazione.getRegione());
	...
	public void addSessioneInfo(String idSessione, SessionePanel sessionePanel) {
		...
		sessionePanel.setDataOraInizio(t.toDateFormat(sessione.getDataInizio().getTime()));
		sessionePanel.setDataOraFine(t.toDateFormat(sessione.getDataFine().getTime()));
	...
	public void addInterventi(String sessione, SessionePanel sessionePanel) {
		...
		for (Intervento i : interventoDAO.getInterventi(sessione)) {
			...
			sessionePanel.getInterventi().add(new InfoInterventoLbl(t.toTimeFormat(i.getOraInizio().getTime()) + " - " + t.toTimeFormat(i.getOraFine().getTime()), ""));
...
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+	
	...
	public void inserisciInfo(Aggiunta2Frame viewAggiuntaConf) throws EmptyFieldException {
	...
	public void createOrganizzatore() {
	...
	public void addSessione(Integer n) throws EmptyFieldException, NoInterventionException, DateTimeException {
		...
		if (ViewSessioni.FieldsAreEmpty()) {
			...
			ViewAggiunta2.getNextBtn().setEnabled(true);
	...
	public void createSessione(int n) {
		
		sessione = new Sessione();
		sessione.setId(IdNextConf + "0" + n);
		...
		if (ViewKeynote == null) { sessione.setKeynoteSpeaker("nessuno"); }
	...
	public void addQueryKeynote() throws EmptyFieldException {
	...
	public void createIntervento(String idSessione, int n) {
		
		intervento = new Intervento();
		
		intervento.setIdIntervento(idSessione + "0" + n);
	...
	public void createPartecipante(String idIntervento) {
...
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
	...
	public void setInfoIntervento() {
		...
		ViewInterventi.setPartecipantiLbl("Aggingi uno o più partecipanti (seleziona il numero): ");
    ...
    public void setSessioneDate(ModSessionePanel sessionePanel, Sessione sessione) {
	...
	public int addModIntervento(String idSessione, ModSessionePanel sessionePanel) {
		
		int n = 0;
		
		for (Intervento i : interventoDAO.getInterventi(idSessione)) {
			
			ModInterventoPanel iPanel = new ModInterventoPanel(i.getIdIntervento(), this);
			
			iPanel.add(new InfoInterventoLbl(t.toTimeFormat(i.getOraInizio().getTime()) + " - " + t.toTimeFormat(i.getOraFine().getTime()), ""));
			iPanel.add(new DescrizioneTextPane(i.getDescrizione()));
			
			for (Partecipante p : partecipanteDAO.getPartecipanti(i.getIdIntervento())) {
				iPanel.add(new InfoInterventoLbl(p.getNome() + " " + p.getCognome(), p.getEmail()));
			}
	...
	public void modificaSessione(ArrayList<ModSessionePanel> modSessioniPanels) {
		...
		for (ModSessionePanel s : modSessioniPanels) {
			...
			queryModificaSessioni += "UPDATE sessioni SET ";
...
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
	...
	public void confermaRimozione(String idConferenza) {
		...
		ViewConferenza.dispose();
		ViewMain.setVisible(true);
		JOptionPane.showMessageDialog(ViewInfo, "Rimozione effetuata.", "", JOptionPane.INFORMATION_MESSAGE);
	}
